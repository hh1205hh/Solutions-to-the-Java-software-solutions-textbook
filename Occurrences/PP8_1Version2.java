/*
Write a program that reads an arbitrary number that are in the range 0 to 50 inclusive
...
public class PP8_1Version2 {
    public static void main(String[] args) {
        ...
        // Initializing the all indexes values in the array to 0
        ...
        while (true) {
            ...
            if (numUser == SENTINEL) {
                break; 
            } else if (numUser < 0 || numUser > RANGE) {
                System.err.println("Error. Please enter integer values in range 0-50 (-1 to exit)");
                numUser = scanner.nextInt();
            // count the amount of the integer value entered so far and increment that value 
            } else {
                int count = occurrences.get(numUser);
                occurrences.set(numUser, count + 1);
            } 
        } 
        ...
        for (int i = 0; i < occurrences.size(); i++) {
            ...
            if (count > 0) {
                System.out.println(i + ": " + count + " occurrence" + (count > 1 ? "s" : ""));
            }
        }
